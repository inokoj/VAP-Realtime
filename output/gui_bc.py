...
def process_client(server_ip='127.0.0.1', port_num=50008):
    ...
    global p_bc_react, time_x3
    global p_bc_emo, time_x4
    #global wavPlay
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock_server:
        ...
        while True:
            try:
                ...
                #print(data['p_now'])
                p_bc_react = np.append(p_bc_react, vap_result['p_bc_react'][0])
                p_bc_react = p_bc_react[-MAX_CONTEXT_LEN:]

                p_bc_emo = np.append(p_bc_emo, vap_result['p_bc_emo'][0])
