...
if __name__ == "__main__":
    ...
    def animate(frame_index):
        ...
        ax5_sub.set_data(time_vad1, vad1)
        ax6_sub.set_data(time_vad2, vad2)


        return ax1_sub, ax2_sub, ax3_sub1, ax3_sub2, ax4_sub1, ax4_sub2, ax5_sub, ax6_sub

    root = tkinter.Tk()
    root.wm_title("Real-time VAP demo")

    fig, ax = plt.subplots(6, 1, tight_layout=True, figsize=(15,13))
    ...
    # VAD 1
    ax5 = ax[4]
    ax5.set_title('Input waveform 1')
    ax5.set_xlabel('Time [s]')

    time_vad1 = np.linspace(-SHOWN_CONTEXT_LEN_SEC, 0, MAX_CONTEXT_LEN)
    vad1 = np.zeros(len(time_vad1))

    ax5.set_ylim(0, 1)
    ax5.set_xlim(-SHOWN_CONTEXT_LEN_SEC, 0)
    ax5_sub, = ax5.plot(time_vad1, vad1, c='y')

    # VAD 2
    ax6 = ax[5]
    ax6.set_title('Input waveform 2')
    ax6.set_xlim(-SHOWN_CONTEXT_LEN_SEC, 0)
    ax6.set_xlabel('Time [s]')

    time_vad2 = np.linspace(-SHOWN_CONTEXT_LEN_SEC, 0, MAX_CONTEXT_LEN)
    vad2 = np.zeros(len(time_vad2))

    ax6.set_ylim(0, 1)
    ax6_sub, = ax6.plot(time_vad2, vad2, c='b')
